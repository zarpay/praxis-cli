...
| Fact | Value |
| --- | --- |
| Reviewers | `v32` (deepseek-v3.2), `mercury` (inception/mercury-2.5) and `counter` (offline `./praxis-providers/word-count.js`); `flash` retired 2026-09-14 (above). **`mercury` is on trial from 2026-09-14**: a diffusion model at ~1,107 tok/s against v32's 30-45s per file, added as a third reviewer rather than swapped in, so its critiques can be judged against v32's cached ones on the same targets before anything is retired. Adding a reviewer touches no existing hash — v32's and counter's caches are untouched, and `mercury` simply starts with 23 not-validated units. No `reasoning` option: it is meaningless for a diffusion model, so it carries `max_tokens` only. **Epoch opened 2026-09-14**: the live reviewers gained `options: { max_tokens: 16000, reasoning: { effort: "low" } }`, re-baselining them. Cause: `flash` spent its whole output budget reasoning and truncated the tool call mid-string on `src/services/apply-refund.ts` — three times, at 607 and 1338 chars, so a varying cut rather than a fixed ceiling. Praxis sends no `max_tokens` of its own, so the ceiling was the backend's. `counter` is deliberately untouched: its hash backs the all-cache-hits canary |
