...
- **Real reviewers are the default for everything judgment-shaped.**
  ...
  so [paid] rows always run with `mercury` — `counter` covers only the
...
| Fact | Value |
| --- | --- |
| Reviewers | `mercury` (inception/mercury-2.5) and `counter` (offline `./praxis-providers/word-count.js`). **`v32` retired 2026-09-14, `flash` the same day** (above). Mercury was added on trial and adopted on evidence: across 23 targets it and v32 agreed on 21, including the **same 6 failures** — every one of the deliberately-violating services plus flavor-of-day and rank-parlors. Both its extra findings were warnings v32 passed; one was real (`award-types.ts` breaks the spec's `<feature>-types.ts` convention, which v32 had never caught), one was a hallucination (claimed `apply-discount.ts` exports `ACTIVE_CODES`, which is `const`, not `export const`). Adopted anyway: this is a demo fixture, ~1,107 tok/s against 30-45s per file is the property being demonstrated, and praxis already handles a false finding — it is a dismissible warning and the dismissal rate is the reviewer-trust signal. Carries `max_tokens` only; `reasoning` is meaningless for a diffusion model |
...
| Corpus units | 23 per reviewer (46 verdicts across two reviewers; briefly 69 across three during the mercury trial, 2026-09-14) |
...
| Known real findings (corpus) | mercury 6 failing + 2 warning, counter 0. The 6: `flavor-of-day`, `rank-parlors.ts`, and the four fixture services. v32 found the same 6 before retiring |
...
| Command | Expect |
| --- | --- |
| `praxis compile` | `Compiled 3 agent(s)` — scooper, sundae, taster; profiles in `agent-profiles/`, plugin output in `plugins/praxis/` (agents + skill) |
...
| `eval run src/services/redeem-coupon.ts --reviewer mercury` [paid on miss] | Fast loop: critiques print **raw** (reviewer prose against the spec — never an `[AX-…]` citation at review time; labels arrive at triage and show in reports); ledger gains a `scope: "files"` run with critiques born `axiom_id: null` |
...
| `eval run --fail-fast --reviewer mercury` [free when warm] | Stops at the first error verdict — flavor-of-day fails and later domains never print (verified 2026-09-08: 5 hits then stop) |
