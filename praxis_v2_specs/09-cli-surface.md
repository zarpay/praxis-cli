...
**Praxis is fully CLI-driven, and agents are first-class users of the CLI.** The agent that just produced a diff validates it by running `praxis validate`; the agent that violated AX-0011 reads the standard by running `praxis axioms show AX-0011`. There is no second interface.
...
```
Typical flows:
  Validate work you just changed:   praxis validate document <path> --json
  Understand a violated standard:   praxis axioms show <id>
  Check project health:             praxis status
  See what a spec covers:           praxis validate all --type <type>
```
...
- `praxis validate all [--type] [--json]` — plus epoch-boundary detection warnings (02)
