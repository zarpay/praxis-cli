...
**It grows a taxonomy.** Recurring critiques triage into **axioms** — named, human-accepted standards with stable ids. Once active, an axiom names the standard: triage labels every recurring critique under it, so the same violation carries the same id and the same words everywhere, `praxis axioms show AX-b951db` explains it with examples, and `praxis eval report` can finally chart it — as a rate with a denominator, per reviewer, never pooled.
