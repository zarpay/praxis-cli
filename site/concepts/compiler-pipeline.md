...
The assembled profile is written to `{agentProfilesOutputDir}/{alias lowercased}.expert.md` — here, `agent-profiles/scooper.expert.md`. Because this expert declares `validates:`, the profile also opens with the eval-targeting frontmatter (`paths:`, `excludes:`) that makes it a working **spec**: the compiled agent and the standard that reviews `src/services/*.ts` are the same file. See [Profiles as spec files](/concepts/agent-profiles#profiles-as-spec-files).
