...
The choices in Praxis that might surprise you — and the reasoning behind them.
...
The tradeoff is that the README must serve two audiences — human readers and the LLM validator. In practice, writing clearly for humans also works well for LLMs. The spec examples in the [Writing Specs](/validation/writing-specs) guide are written this way.
...
Schema validators can check field presence and type. They cannot check whether a `description` field is actually descriptive, whether a `## Scope` section actually explains the agent's scope, or whether a responsibility document makes sense given the spec's guidelines.
