...
The axiom layer (04, review→label) sits on the ledger: critiques are born raw (the reviewer sees only the spec — axioms never enter any prompt about the code), `axioms triage` labels the pending backlog via the curator — one call per critique, so ordering can't bias a verdict (assignment records, matcher provenance, human-overridable), and `axioms curate` is the interactive session for the residue — clustering into proposals (ids `AX-` + 6 random hex, never sequential), dismissals, manual assignments. Ratification traces a proposal to its spec, records the derivation (`derived_from` — provenance, never identity), and activates it, with no cache effect. Effective labels are decided at read time by `join-critique-labels` (newest record wins). The `curator` config role (required for triage/curate/ratify/audit; instructive error otherwise) runs the labeling, organizing, the authoring gate (03), and traceability via the provider's optional `complete()`. Decisions append to `.praxis/ledger/triage/`.
