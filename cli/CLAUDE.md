...
- **Terminal output:** all output goes through `@/core/logger.js` — `Display` renders a command's stdout output (`lines([...])` with color tuples and falsy-skipped conditional entries, `badge()`, `header()`); `Logger` writes stderr diagnostics. Raw `console.*` is banned outside that module (ESLint `no-console`).
