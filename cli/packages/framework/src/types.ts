...
/**
 * The one signature every prompt has.
 *
 * Generix in it's context: the framework fixes the shape - a template (string)
 * and it's template-able variables, if any are required.
 */
export type Prompt<Options = NoOptions> = (
  variables: Options,
) => string;
