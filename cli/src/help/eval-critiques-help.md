...
Behavior:
  ...
  invalid in `eval review`), advisory (born from `praxis feedback` —
  never queued, never reported). The ids printed here are exactly what
  reassign and review take.

JSON (--json), stable contract:
  { totals { untriaged, unmatched, labeled, dismissed, advisory },
