...
2. **propose** — the cluster reveals a standard no axiom names yet AND the specification's text supports it. Draft the axiom at the altitude of one remediation — one fix resolves every critique in the cluster: a one-to-three sentence statement of what it asserts, a violating and a compliant example (drawn from or modeled on the critiques), a severity, and the spec passage that grounds it, quoted verbatim. Draft only what needs reading comprehension to decide. If a regex or AST query could decide the standard with zero false positives, or two senior engineers could never disagree on a verdict, it is mechanical: it belongs in static tooling, so hold the cluster and say so. When a cluster mixes a mechanical half with a judgment half — "declares a required field" beside "the payload is a complete snapshot" — draft the judgment half alone. Never draft a twin: if an established axiom's fix would resolve the cluster, assign instead.
