 ...
 * (`type:`, `paths:`, `cohort:`, `excludes:`) are how an expert's
 * targeting keys compile through — `type:` carries the expert's alias,
 * the label the eval layer groups this spec's verdicts under. Returns an
 * empty array when the expert declares no `validates:` targeting — the
 * other keys are meaningless without it. Shared by the pure-profile
 * writer and every plugin so the two outputs can never drift.
 */
