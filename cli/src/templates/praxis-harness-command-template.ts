/**
 * The /praxis-harness slash command written into user projects: an
 * agent-facing prompt that reads the harness brief and drafts harness
 * edits as a reviewable PR. Praxis emits the brief and stops there
 * (08): the agent drafts, a human ratifies — Praxis never edits the
 * harness.
 */
export default function praxisHarnessCommandTemplate(): string {
  return `---
