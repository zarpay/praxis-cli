...
**Advisory critiques never queue** (2026-09-14). A `praxis feedback` run is recorded like any other — its critiques, its cost, full provenance — but stamped `scope: "advisory"`, and triage skips those critiques, so curate never sees them either. They describe code in flight; queueing them would bury the critiques about code that landed under the ones about code that was never finished. `eval critiques` still lists them, because browsing the ledger is its job — under their own state, **advisory** (2026-09-15): a critique that was never destined for a queue is not "untriaged", and calling it that promised work nobody could do while making the browse count disagree with the queue's own; `eval review` does not offer them, because a critique that is evidence for nothing cannot usefully be judged invalid.
