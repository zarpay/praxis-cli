...
1. **Coverage and conformance render together, always.** A conformance figure without its coverage denominator is not printed. Rising conformance + falling coverage renders as a warning, not a win. (Implemented 2026-09-22: **eval coverage** — two slices over every file under `sources` minus `ignore` — renders in `praxis status`, in the corpus summary of `eval run` and `eval ci`, and in their `--json` payloads as `coverage { sourceFiles, observed { files, rate, display }, passing { files, rate, display } }`. `observed` is structural — files any spec governs, derived from the eval layer's own discovery, accurate before the first run. `passing` is the verdict-backed score a CI holds a bar against — files whose recorded verdict is compliant from **every** configured reviewer (agreement is the bar, never an average — rule 7's spirit for a single number): ungoverned, unreviewed, warned, and failed files all count against it. Spec files, templates, and the authored taxonomy (experts, practices, compiled profiles) are direction rather than corpus and count on neither side; a spec's `excludes:` leave files in the corpus but unobserved. A census, not a sample: the small-n floor does not apply.)
...
4. ~~**The calibration banner appears on every report.**~~ (Calibration withdrew to the roadmap 2026-09-07 and the banner stood in for it; removed 2026-09-23 — a permanent static warning is a ghost of an unshipped feature, not a measurement. The `calibration` fields left every report payload and `calibration_stale` left the status poll; the rule returns, real, with M6.)
...
- Violation flow / introduction rate: withdrawn to the roadmap (2026-09-07) with the diff unit.
- Residual rate (04: the dismissal rate — the reviewer-trust signal)
