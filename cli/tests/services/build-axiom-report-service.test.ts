...
describe("buildAxiomReportService", () => {
  ...
  it("reports one row per reviewer with representative critiques by ledger id", () => {
  ...
  it("throws the instructive error for an unknown axiom id", () => {
    seedRun("r1", "flash", ["docs/a.md"]);
    const cfg = testConfig(root, { reviewers: [{ name: "flash", model: "m", apiKeyEnvVar: "K" }] });
    const scoped = resolveReportScopeService(cfg, {});

    const build = () => buildAxiomReportService(cfg, { scoped, axiomId: "AX-000000" });

    expect(build).toThrow("praxis axioms list");
  });
});
