...
2. **Every mechanical criterion kept out of the judge is a class of judge error made impossible** — and a class of calibration burden removed. The tiered design achieved this by routing; the boundary achieves it by admission control, which is cleaner.
...
An exclusion stated in prose is an instruction the judge must notice and obey (observed failure: the events SME excludes `ApplicationEvent` in bold prose; the judge failed it with six errors while acknowledging the exclusion in its own critique text). An exclusion in frontmatter is a file the judge never receives. Prevention beats calibration wherever prevention is available. `exemplars` serve double duty: excluded from adverse judgment, and available as few-shot positives and calibration seed cases (06).

**Judgment axioms** (`mode: judgment`, the default). The LLM judge, for questions that need reading: quality of descriptions, richness of payloads, "is this business logic." Carries the full apparatus — provenance, caching, calibration, drift tracking. Judgment input is always holistic per the axiom's declared scope — the judge reads files, never bare diffs (01).

**Agentic judgment** (`mode: agentic`; opt-in, deferred). A judge with read tools that explores the codebase the way the interactive SME agent does — closing the epistemic gap between the SME-as-agent (has tools) and the SME-as-pipeline-judge (one-shot prompt). Strictly more capable than `file+context` where relevant context can't be statically declared. Costs are structural: spend becomes unbounded-ish, variance rises, provenance requires logging every tool call, calibration gets harder. An axiom must opt in explicitly, making its expense visible by declaration. Nothing initial requires this mode; it exists in the schema so `file+context` isn't silently stretched into pseudo-agentic behavior.

## Scope declarations

Defined in 01, owned here: `hunk` (decidable from the change alone; opt-in cost optimization), `file` (default), `file+context` (spec-declared extra files inlined into the judgment prompt — and into the content hash, or provenance breaks), `cohort` (relational properties of a declared set — completeness, orphans, cross-file consistency; judged over the whole set, keyed on a cohort hash of member list + member hashes; boundaries declared via `cohort: by_directory | glob`, never inferred), `changeset` (judged from the whole diff across touched files; where propagation-failure axioms live, which is where agent failure modes concentrate).

**The governing rule: the judge's context contains exactly what the axiom is about.** Less is myopia (the `file+context` cases); more is contamination — a judge shown 30 files while evaluating one normalizes file B's violation against file A's pattern, or invents consistency requirements the spec never states. The same effect that is a bug for file axioms is the *feature* for cohort axioms: seeing the set together is the only way to judge a relational property.

**Batching is prohibited for file-scoped axioms.** The tempting middle — N files, one call, structured per-file verdicts — fails on provenance, not cost: batch-mates are part of the judgment input, so a per-file cache entry extracted from a batched call is not reproducible per-file. Recording the batch-mates fixes provenance but makes cache hits require identical batches, which makes the cache useless. Prompt caching of the shared spec prefix captures the savings legitimately; per-file calls also keep the judge's full attention on one unit (a 30-file context invites lost-in-the-middle degradation).
...
The spec document keeps its mechanical content — it serves human readers, and those rules are still the team's standards. But the **judgment contract is the axiom set** (04): the judge is asked about admitted judgment axioms only, and the SME's enforcement posture states that mechanical criteria are out of scope and must not be evaluated.

This directly removes the surface the observed hallucinations grew on: the judge re-deriving naming and inheritance checks badly (flagging the spec's own canonical example as a naming violation; inventing a "recommended async queue" criterion). A judge that is never asked mechanical questions cannot answer them wrongly.
...
2. Judgment axioms evaluate per their declared scope; the judge prompt carries the axiom set, the exemplars, and the statement that mechanical criteria are out of scope.
...
4. For diff-unit evaluation (01), before/after verdicts feed **verdict diffing** — attribution is computed set-difference over axiom-anchored results on `(axiom_id, location/symbol)` under shared provenance, never a judge task.
