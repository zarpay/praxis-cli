...
Critiques become durable records. The ledger (05): run + critique records, full provenance, OpenRouter `usage` captured (cost data exists for the first time). Cache format changes: reviewer-hash namespacing, context-in-key. Reviewer output becomes structured critiques ready for the two channels. Corpus-level only — no git, no axioms yet. *Standalone value: durable evidence and cost visibility on any v1 project, immediately.*
