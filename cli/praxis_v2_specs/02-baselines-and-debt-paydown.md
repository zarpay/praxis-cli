...
- **Struggle** — the _introduction rate_ per axiom in new work within an epoch. "The harness consistently struggles with payload richness" is a precise sentence: AX-0011's introduction rate stays flat across N diffs while other axioms' rates decline. Consistency of struggle across many diffs is what licenses the word "harness" — a single bad diff licenses nothing.
...
- Epoch boundaries are first-class, _named_ events in every report: "model → sonnet-4.6", "events spec v3".
...
**The cost of changing a standard, stated plainly for spec authors:** editing a spec spends the trend line of every axiom it touches. This is not a reason to freeze specs — standards should improve — it is a reason to (a) batch spec edits rather than dribbling them, (b) version axioms individually (04) so _untouched_ axioms keep their history across a spec edit, and (c) re-baseline immediately. Axiom-level versioning is what makes spec evolution affordable: the hard break is per-axiom, not per-spec.

Per-axiom epochs also resolve cleanly with 01's per-axiom population clocks: a new axiom added to an old spec opens its own epoch with its own baseline, and all existing code is debt _with respect to that axiom_ — nobody's introduction rate is retroactively poisoned.
...
An org that wants the human/agent contrast can have it — by _declaring_ its conventions, never by inference:
...
- Announce the boundary loudly and _name_ it ("model → sonnet-4.6", "events spec v3") — this is the label reports use.
- Recommend a re-baseline; auto-set `baseline: true` when the epoch-opening run is a full `eval run`.
- **Warn, never block.**

Implementation refinement (2026-09-02): detection is **set-wise per reviewer name**, not last-run-wise — a boundary is a hash absent from the reviewer's entire run history, and `baseline: true` is the first _corpus_ run under a hash no prior corpus run carries. Contributors (or CLI versions) interleaving runs under known hashes therefore never re-announce, and branch merges cannot flip a baseline. A reviewer with no history is bootstrap, not a boundary — but its first full run still opens its first epoch as baseline. Spec-hash boundary _naming_ is deferred to per-axiom epochs (M4); the cache already enforces spec-edit invalidation structurally.
