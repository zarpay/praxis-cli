...
`frontmatter`, `frontmatter-fields`, `paths`, `spec-pattern`. It depends on no
...
- `errors.ts` owns every error Praxis raises: one factory per failure mode,
  ...
  build error strings.
- `frontmatter.ts` is the only implementation of the YAML/delimiter format,
  including `body()`. Nothing re-scans for `---`.
